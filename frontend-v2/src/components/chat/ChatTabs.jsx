...
// Individual tab component to handle its own data fetching
const TabItem = ({ tab, isActive, onClick, onClose }) => {
    // Only fetch for direct chats with a valid otherUserId
    const shouldFetch = tab.type === 'DIRECT' && !!tab.otherUserId;

    const { data: userData } = useQuery({
        queryKey: ['user', tab.otherUserId],
        queryFn: () => userService.getUserById(tab.otherUserId),
        enabled: shouldFetch,
    ...
    return (
        <Motion.div
            ...
            onClick={onClick}
        >
            {tab.type === 'DIRECT' || tab.type === 'direct' ? (
                <Avatar
                    src={tab.avatar}
                    name={tab.title}
                    size="xs"
                    online={isOnline}
                    showOffline={!isOnline && shouldFetch}
                />
            ) : (
...
export function ChatTabs() {
    ...
    // Restore active conversation when tabs are loaded from localStorage
    useEffect(() => {
        if (activeTabId && tabs.length > 0) {
            const activeTab = tabs.find(tab => tab.id === activeTabId);
            if (activeTab) {
                setActiveConversation(activeTab.conversationId);
            }
        }
    }, [tabs, activeTabId, setActiveConversation]);
